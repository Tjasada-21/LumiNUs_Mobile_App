...
const doesEventOccurOnDate = (event, targetDate) => {
  ...
  const endDate =
    rawEndDate && !Number.isNaN(rawEndDate.getTime()) ? rawEndDate : startDate;
...
const EventsScreen = ({ navigation }) => {
  ...
  const [registeredEventsLoading, setRegisteredEventsLoading] =
  ...
  const eventDayCountMap = React.useMemo(
    () => buildEventDayCountMap(events, currentYear, currentMonth),
  ...
  const selectedCalendarDayOverflowCount = Math.max(
    selectedDayEvents.length - selectedCalendarDayEventPreview.length,
    0,
  );
  ...
  const fetchRegistrations = React.useCallback(async () => {
    try {
      ...
      try {
        ...
        if (isMountedRef.current) {
          setRegisteredEventsError(
            "Unable to load your registered events right now.",
          );
          setRegisteredEvents([]);
        }
      }
  ...
  const openRegisteredEvent = (registration) => {
    ...
    const fullEventFromList =
      events.find(
        (event) => Number(event?.id) === Number(registeredEvent?.id),
      ) ?? null;
    ...
    const normalizedRegisteredEvent = registeredEvent
      ? {
          ...eventToOpen,
          cover_image_url:
            eventToOpen?.cover_image_url ??
            eventToOpen?.images?.[0]?.image_path ??
            null,
        }
      : null;
    ...
    if (rootNavigator?.navigate) {
      rootNavigator.navigate("ViewEventsScreen", {
        event: normalizedRegisteredEvent,
      });
      return;
    }

    if (parentNavigator?.navigate) {
      parentNavigator.navigate("ViewEventsScreen", {
        event: normalizedRegisteredEvent,
      });
      return;
    }
      ...
      <View style={styles.container}>
        ...
        >
          <View style={styles.searchRow}>
            ...
            <TouchableOpacity
              style={[styles.squareButton, styles.accentButton]}
              activeOpacity={0.85}
          ...
          >
            {eventsLoading ? (
              <View style={styles.featuredEventLoadingCard}>
                ...
                <Text style={styles.featuredEventLoadingText}>
                  Loading events...
                </Text>
              </View>
                ...
                return (
                  ...
                  >
                    ...
                    <View style={styles.featuredEventBody}>
                      <Text numberOfLines={1} style={styles.featuredEventTitle}>
                        {event.title}
                      </Text>
                      <Text
                        numberOfLines={2}
                      ...
                      <View style={styles.featuredEventMetaRow}>
                        <Text
                          numberOfLines={1}
                          style={styles.featuredEventMetaText}
                        >
                      ...
                      <Pressable
                        style={styles.cardButtonPill}
                        onPress={() => openEvent(event)}
                      >
                        <Text style={styles.cardButtonPillText}>
                          View Event
                        </Text>
                      </Pressable>
                    </View>
              ...
              <View style={styles.featuredEventEmptyCard}>
                <Text style={styles.featuredEventEmptyTitle}>
                  No events available.
                </Text>
          ...
          >
            <View style={styles.calendarOverlay}>
              ...
              <View style={[styles.calendarCard, { width: calendarWidth }]}>
                ...
                <View
                  style={styles.calendarPagerWindow}
                  ...
                  >
                    ...
                    <View style={styles.calendarGrid}>
                      {monthDays.map((day, index) => {
                        ...
                        const eventCount = day
                          ? (eventDayCountMap.get(day) ?? 0)
                          : 0;
                        const hasEvent = eventCount > 0;
                        const bubbleAnchorStyle =
                          getCalendarBubbleAnchorStyle(index);
                              ...
                              <>
                                <Pressable
                                  style={[
                                    ...
                                    isToday &&
                                      hasEvent &&
                                      styles.calendarDayBubbleTodayEvent,
                                    selectedCalendarDay === day &&
                                      styles.calendarDayBubbleSelected,
                                  ]}
                                ...
                                >
                                  <Text
                                    style={[
                                      ...
                                      isToday &&
                                        hasEvent &&
                                        styles.calendarDayTextTodayEvent,
                                      selectedCalendarDay === day &&
                                  ...
                                  {hasEvent ? (
                                    <View
                                      style={[
                                        ...
                                        isToday && styles.calendarEventDotToday,
                                      ]}
                                    />
                                  ) : null}
                                </Pressable>
                                ...
                                selectedDayEvents.length > 0 ? (
                                  <Animated.View
                                    style={[
                                      styles.calendarDayEventBubble,
                                      bubbleAnchorStyle.bubble,
                                      bubbleAnimatedStyle,
                                    ]}
                                  >
                                    {selectedCalendarDayEventPreview.map(
                                      (event) => (
                                        <Pressable
                                          key={`calendar-day-bubble-${event.id}`}
                                          style={({ pressed }) => [
                                            styles.calendarDayEventBubbleItemRow,
                                            pressed
                                              ? styles.calendarDayEventBubbleItemRowPressed
                                              : null,
                                          ]}
                                          onPress={() => {
                                            closeCalendar();
                                        ...
                                        >
                                          <Image
                                            ...
                                            style={
                                              styles.calendarDayEventBubbleThumb
                                            }
                                            resizeMode={
                                              event.cover_image_url
                                                ? "cover"
                                          ...
                                          <Text
                                            numberOfLines={2}
                                            style={
                                              styles.calendarDayEventBubbleItem
                                            }
                                          >
                                            {event.title ?? "Event"}
                                          </Text>
                                    ...
                                    {selectedCalendarDayOverflowCount > 0 ? (
                                      <Text
                                        style={
                                          styles.calendarDayEventBubbleMore
                                        }
                                      >
                                        +{selectedCalendarDayOverflowCount} more
                                      </Text>
                                    ) : null}
                                    <View
                                      style={[
                                        styles.calendarDayEventBubbleArrow,
                                        bubbleAnchorStyle.arrow,
                                      ]}
                                    />
                                  </Animated.View>
                                ) : null}
          ...
          <Modal
            transparent
            visible={registrationsVisible}
            animationType="fade"
            onRequestClose={closeRegistrations}
          >
            <View style={styles.calendarOverlay}>
              <Pressable
                style={styles.calendarBackdrop}
                onPress={closeRegistrations}
              />
              <View style={styles.registrationCard}>
                ...
                {registeredEventsLoading ? (
                  ...
                  <ScrollView
                    showsVerticalScrollIndicator={false}
                    contentContainerStyle={styles.registrationList}
                  >
                    {registeredEvents.map((registration) => (
                      <Pressable
                        ...
                        style={({ pressed }) => [
                          ...
                          pressed ? styles.registrationItemPressed : null,
                        ]}
                      ...
                      >
                        ...
                        <View style={styles.registrationTextWrap}>
                          <Text style={styles.registrationItemTitle}>
                            {registration.event?.title ?? "Registered Event"}
                          </Text>
                          <Text style={styles.registrationItemMeta}>
                            {formatRegisteredEventDate(registration)} •{" "}
                            {getRegisteredEventLocationLabel(registration)}
                          </Text>
                        </View>
                      </Pressable>
                    ))}
                  ...
                  <View style={styles.registrationEmptyState}>
                    ...
                    <Text style={styles.registrationEmptyText}>
                      {registeredEventsError ||
                        "You have no pre-registered events yet."}
                    </Text>
                    <TouchableOpacity
                      style={styles.registrationOkayButton}
                      activeOpacity={0.85}
                      onPress={closeRegistrations}
                    >
                      <Text style={styles.registrationOkayButtonText}>
                        Okay
                      </Text>
                    </TouchableOpacity>
                  </View>
          ...
          <ScrollView
            horizontal
            showsHorizontalScrollIndicator={false}
            contentContainerStyle={styles.comingSoonRow}
          >
