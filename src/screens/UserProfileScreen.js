...
import { View, Text, Image, TouchableOpacity, ActivityIndicator, ScrollView, RefreshControl, useWindowDimensions, Modal, TextInput, KeyboardAvoidingView, Platform, Animated } from 'react-native';
...
const UserProfileScreen = ({ navigation }) => {
  ...
  const saveWorkExperience = async () => {
    ...
    if (!workDraft.title.trim()) {
      ThemedAlert.alert('Missing field', 'Please enter a position/title.', [{ text: 'OK' }], { variant: 'error' });
      return;
    }
    if (!workDraft.subtitle.trim()) {
      ThemedAlert.alert('Missing field', 'Please enter a company/organization.', [{ text: 'OK' }], { variant: 'error' });
      return;
    }
    if (!workDraft.location.trim()) {
      ThemedAlert.alert('Missing field', 'Please enter a location.', [{ text: 'OK' }], { variant: 'error' });
      return;
    }
    ...
    try {
      ...
      if (!supaUser) {
        ThemedAlert.alert('Sign in required', 'Please sign in again before updating work experience.', [{ text: 'OK' }], { variant: 'error' });
        return;
      }
      ...
      await loadProfile();
      setIsWorkModalVisible(false);
      ThemedAlert.alert('Work experience saved', 'Your work experience was updated.', [{ text: 'OK' }], { variant: 'success' });
    } catch (err) {
      ...
      ThemedAlert.alert('Save failed', errorMsg, [{ text: 'OK' }], { variant: 'error' });
    } finally {
        ...
        { text: 'Cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              const supaUser = await getCurrentUser();
              if (!supaUser) {
                ThemedAlert.alert('Sign in required', 'Please sign in again before deleting work experience.', [{ text: 'OK' }], { variant: 'error' });
                return;
              }
              ...
              await loadProfile();
              ThemedAlert.alert('Deleted', 'Work experience removed.', [{ text: 'OK' }], { variant: 'success' });
            } catch (err) {
              console.error('Failed to delete work experience:', err);
              ThemedAlert.alert('Delete failed', 'Unable to delete work experience right now.', [{ text: 'OK' }], { variant: 'error' });
            } finally {
              setIsWorkSaving(false);
            }
          },
  ...
  const saveBiography = async () => {
    ...
    try {
      ...
      const updated = await updateAlumniProfile(supaUser.id, { alumni_bio: nextBio || null }).catch((err) => {
  ...
  const savePostUpdate = async (payload, successTitle, successMessage) => {
    ...
    try {
      ...
      const supaUser = await getCurrentUser();
      if (!supaUser) {
        ThemedAlert.alert('Sign in required', 'Please sign in again before updating a post.', [{ text: 'OK' }], { variant: 'error' });
        return;
      }
      ...
      setIsPostActionModalVisible(false);
      setPostActionPost(null);
      ThemedAlert.alert(successTitle, successMessage, [{ text: 'OK' }], { variant: 'success' });
    } catch (error) {
      console.error('Failed to update post:', error);
      ThemedAlert.alert('Update failed', 'Unable to update the post right now.', [{ text: 'OK' }], { variant: 'error' });
    } finally {
        ...
        { text: 'Cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              const supaUser = await getCurrentUser();
              if (!supaUser) {
                ThemedAlert.alert('Sign in required', 'Please sign in again before deleting a post.', [{ text: 'OK' }], { variant: 'error' });
                return;
              }
              ...
              setIsPostActionModalVisible(false);
              setPostActionPost(null);
              ThemedAlert.alert('Post deleted', 'Your post has been removed.', [{ text: 'OK' }], { variant: 'success' });
            } catch (error) {
              console.error('Failed to delete post:', error);
              ThemedAlert.alert('Delete failed', 'Unable to delete the post right now.', [{ text: 'OK' }], { variant: 'error' });
            } finally {
              setIsPostActionSaving(false);
            }
          },
                          ...
                          <View style={styles.workCard}>
                            <View style={styles.workRow}>
                              ...
                              <View style={styles.workActionsRow || { justifyContent: 'flex-end' }}>
                                ...
                                <TouchableOpacity style={[styles.workDeletePill, { marginLeft: 8 }]} activeOpacity={0.8} onPress={() => handleDeleteWork(emp)}>
                                  <Ionicons name="trash-outline" size={15} color="#B91C1C" />
                                  <Text style={styles.workDeletePillText}>Delete</Text>
                                </TouchableOpacity>
                              </View>
                            </View>
                          </View>
