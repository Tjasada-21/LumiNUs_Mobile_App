  ...
  try {
    ...
    if (error) {
      ...
      if (errMsg.includes("RLS") || errMsg.includes("policy")) {
        console.error(
          "[events] ❌ RLS Policy Error - Run SUPABASE_RLS_DISABLE.sql in Supabase",
        );
      } else if (errMsg.includes("does not exist")) {
        console.error(
          "[events] ❌ Table does not exist - Check Supabase database schema",
        );
      } else {
    ...
    if (errMsg.includes("RLS")) {
      console.error(
        "[events] ❌ RLS POLICY ERROR: Row-Level Security is blocking queries. Run SUPABASE_RLS_DISABLE.sql",
      );
    } else if (errMsg.includes("Unauthorized")) {
      console.error(
        "[events] ❌ UNAUTHORIZED: Check that your Supabase credentials are correct",
      );
    } else {
...
export const getUserEventRegistrations = async (alumniId) => {
  try {
    ...
    console.error(
      "[registrations] Get user registrations error:",
      error.message,
    );
    throw error;
  }
