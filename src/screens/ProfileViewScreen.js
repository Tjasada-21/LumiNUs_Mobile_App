  ...
  const handleAddConnection = async () => {
    ...
    try {
      ...
      if (!currentUser?.id) {
        ThemedAlert.alert(
          "Connection failed",
          "Please sign in again to send a connection request.",
        );
        return;
      }
      ...
      setUserData((current) =>
        current ? { ...current, connection_status: "pending" } : current,
      );
      ...
      ThemedAlert.alert(
        "Connection failed",
        "Unable to add this connection right now.",
      );
    } finally {
    ...
    try {
      ...
      if (!currentUser?.id) {
        ThemedAlert.alert(
          ...
          "Please sign in again to update this request.",
        );
        return;
      }
      ...
      setUserData((current) =>
        current ? { ...current, connection_status: "none" } : current,
      );
      ...
      console.error(
        "Failed to cancel pending request (Supabase):",
        cancelError,
      );
      ThemedAlert.alert(
        "Cancel request failed",
    ...
    try {
      ...
      if (!currentUser?.id) {
        ThemedAlert.alert(
          ...
          "Please sign in again to update this connection.",
        );
        return;
      }
      ...
      setUserData((current) => {
        ...
        return {
          ...
          connections_count: Math.max(
            0,
            Number(current.connections_count ?? 0) - 1,
          ),
        };
      });
      ThemedAlert.alert(
      ...
      ThemedAlert.alert(
        "Remove connection failed",
        "Unable to remove this connection right now.",
      );
    } finally {
      setFollowLoading(false);
    }
    ...
    ThemedAlert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Remove",
          style: "destructive",
          onPress: () => handleRemoveConnectionConfirmed(),
        },
      ],
    );
  };
    ...
    ThemedAlert.alert(
      ...
      [
        { text: "Keep Pending", style: "cancel" },
        {
          text: "Cancel Request",
          style: "destructive",
          onPress: () => handleCancelPendingRequestConfirmed(),
        },
      ],
    );
  };
    ...
    return (
      <View style={styles.profilePostImagesGrid}>
        {visibleImages.map((image, imageIndex) => (
          <View
            key={image?.id ?? `${imageIndex}-${getPostImageUri(image)}`}
            style={[
              styles.profilePostImageTile,
              imageIndex === 0 && visibleImages.length === 1
                ? styles.profilePostImageTileSingle
                : null,
              imageIndex > 0 ? styles.profilePostImageTileWithGap : null,
            ]}
          >
            <Image
                ...
                <View style={styles.heroActionsRow}>
                  <TouchableOpacity
                    style={[
                      ...
                      !isPendingConnection &&
                        ...
                        styles.addConnectionButtonDisabled,
                    ]}
                    activeOpacity={0.85}
                    onPress={
                      isConnected
                  ...
                  <TouchableOpacity
                    style={[
                      ...
                      !isConnected && styles.messageButtonDisabled,
                    ]}
                    activeOpacity={0.85}
                  ...
                  >
                    {workExperiences.map((experience, index) => (
                      <View
                        key={
                          experience.id ??
                          `${experience.title}-${experience.period}-${index}`
                        }
                        style={[
                          styles.workPage,
