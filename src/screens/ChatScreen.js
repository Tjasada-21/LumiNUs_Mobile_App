...
const ChatScreen = ({ navigation }) => {
  ...
  const [isGroupActionModalVisible, setIsGroupActionModalVisible] =
    useState(false);
  ...
  const openGroupConversation = (groupChat) => {
    ...
    const groupMembers = Array.isArray(groupChat?.members)
      ? groupChat.members
      : [];
  ...
  const loadChatData = useCallback(async () => {
    try {
      ...
      const nextGroupChats = Array.isArray(groupChatsData)
        ? groupChatsData
        : [];
  ...
  const handleLoadMore = async () => {
    ...
    if (isFetchingMore || !hasMoreChats || isLoadingChatData || !userData?.id)
      return;
        ...
        setContacts((prevContacts) => {
          ...
          newConversations.forEach((conversation) => {
            ...
            connectionsMap.set(conversation.id, {
              ...
              email: conversation.email ?? baseContact.email,
              alumni_photo:
                conversation.alumni_photo ?? baseContact.alumni_photo,
            });
          });
  ...
  const sortChatsByLatestMessage = (chats) => {
    return chats.sort((firstItem, secondItem) => {
      const firstConversationId =
        firstItem?.group_chat_id ?? firstItem?.connection_id ?? firstItem?.id;
      const secondConversationId =
        secondItem?.group_chat_id ??
        secondItem?.connection_id ??
        secondItem?.id;

      const firstViewedAt =
        conversationViewTimestamps[firstConversationId] ?? 0;
      const secondViewedAt =
        conversationViewTimestamps[secondConversationId] ?? 0;
  ...
  const activeChats = useMemo(() => {
    if (selectedTab === "channels") {
      const channelChats = groupChats.map((groupChat) => ({
        ...groupChat,
        __chatType: "group",
      }));
    ...
    selectedTab,
    favoriteContactIds,
    conversationViewTimestamps,
  ]);

  const tabCounts = useMemo(() => {
    const favoritesCount = contacts.filter((item) =>
      favoriteContactIds.has(item?.id),
    ).length;
      ...
      <View style={styles.skeletonListWrap}>
        {Array.from({ length: 6 }).map((_, index) => (
          <View key={`chat-skeleton-${index}`} style={styles.skeletonCard}>
            ...
            <Animated.View
              pointerEvents="none"
              style={[styles.skeletonShimmer, shimmerTranslateStyle]}
            />
          </View>
        ))}
      </View>
  ...
  const renderContactItem = ({ item }) => {
    ...
    const unreadCount = Number(
      item?.unread_count ?? (item?.is_read === false ? 1 : 0),
    );
    const latestMessage =
        ...
        <View style={styles.contactTextWrap}>
          <View style={styles.contactTopRow}>
            ...
            {messageTimestampLabel ? (
              <Text style={styles.contactTimestamp} numberOfLines={1}>
                {messageTimestampLabel}
              </Text>
            ) : null}
          </View>
  ...
  const handleCreateGroup = async () => {
    ...
    if (parentNavigator?.navigate)
      parentNavigator.navigate("CreateGroup", params);
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            setContacts((prev) =>
              prev.filter((c) => c.id !== modalContact?.id),
            );
          },
        },
      ],
  ...
  const handleArchiveGroup = async () => {
    ...
    try {
      await supabase
        ...
        .eq("group_chat_id", modalGroup?.id)
  ...
  const handleAddMembers = () => {
    ...
    if (parentNavigator?.navigate)
      parentNavigator.navigate("EditGroup", params);
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              setGroupChats((prev) =>
                prev.filter((g) => g.id !== modalGroup?.id),
              );
            } catch (e) {}
          },
        },
  ...
  const renderAdminItem = ({ item }) => {
    const adminName =
      `${item?.admin_first_name ?? item?.first_name ?? ""} ${item?.admin_last_name ?? item?.last_name ?? ""}`.trim() ||
      "Admin";
    const adminAvatar = getAvatarUri(
      adminName,
    ...
    return (
      <Pressable
        ...
        onPress={() =>
          openConversation({
            id: item?.id,
            first_name: item?.admin_first_name ?? item?.first_name,
            last_name: item?.admin_last_name ?? item?.last_name,
            alumni_photo: item?.photo ?? item?.admin_photo,
          })
        }
        onLongPress={() => {
          setModalContact({
  ...
  const getListEmptyComponent = useCallback(() => {
    ...
    if (selectedTab === "channels") {
      return renderEmptyState(
        "No group chats yet.",
        "Create a group conversation and it will appear here.",
      );
    }
    ...
    if (selectedTab === "favorites") {
      return renderEmptyState(
        "No favorites yet.",
        "Mark a chat as a favorite to keep it here.",
      );
    }

    return renderEmptyState(
      "No contacts yet.",
      "Accepted connections will appear here as chat contacts.",
    );
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
        ...
        <View style={styles.contentWrap}>
              ...
              <Pressable
                ...
                onPress={() =>
                  openConversation({
                    id: admins[0]?.id,
                    first_name:
                      admins[0]?.admin_first_name ?? admins[0]?.first_name,
                    last_name:
                      admins[0]?.admin_last_name ?? admins[0]?.last_name,
                    alumni_photo: admins[0]?.photo ?? admins[0]?.admin_photo,
                  })
                }
              >
                <Image
                  source={{
                    uri: getAvatarUri(
                      (admins[0]?.admin_first_name ?? admins[0]?.first_name) ||
                        "Admin",
                      admins[0]?.photo ?? admins[0]?.admin_photo,
                    ),
                  }}
                ...
                <View style={styles.contactTextWrap}>
                  <Text style={styles.contactName} numberOfLines={1}>
                    {`${admins[0]?.admin_first_name ?? admins[0]?.first_name ?? ""} ${admins[0]?.admin_last_name ?? admins[0]?.last_name ?? ""}`.trim() ||
                      "Admin"}
                  </Text>
            ...
            {TABS.map((tab) => {
              ...
              return (
                <Animated.View
                  key={tab.key}
                  style={[
                    styles.segmentItemMotionWrap,
                    getTabAnimatedStyle(tab.key),
                  ]}
                >
                  <TouchableOpacity
                    style={[
                    ...
                    <View style={styles.segmentInnerRow}>
                      ...
                      <View
                        style={[
                          ...
                          isActive && styles.segmentCountPillActive,
                        ]}
                      >
                        <Text
                          style={[
          ...
          <Animated.View style={[styles.listArea, tabContentAnimatedStyle]}>
            <FlatList
              ...
              keyExtractor={(item) =>
                `${item?.__chatType ?? "chat"}-${String(item?.group_chat_id ?? item?.connection_id ?? item?.id)}`
              }
              showsVerticalScrollIndicator={false}
              contentContainerStyle={styles.listContent}
      ...
      <Modal
        ...
        onRequestClose={hideContactActions}
      >
          ...
          <SafeAreaView style={styles.actionSheetSafeArea} edges={["bottom"]}>
            <View style={styles.actionSheet}>
              ...
              <Pressable
                ...
                onPress={() => handleToggleFavorite(modalContact?.id)}
              >
                <Ionicons
                  name="star-outline"
                  size={20}
                ...
                />
                <Text style={[styles.actionText, styles.favoriteText]}>
                  Favorite
                </Text>
              </Pressable>
              ...
              <Pressable style={styles.actionItem} onPress={handleCreateGroup}>
                ...
                >{`Create group chat with '${modalContact?.first_name ?? ""}'`}</Text>
              </Pressable>
      ...
      <Modal
        ...
        animationType={"slide"}
        statusBarTranslucent={true}
        onRequestClose={hideGroupActions}
      >
