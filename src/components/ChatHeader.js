...
const ChatScreen = ({ navigation }) => {
  ...
  const loadChatData = useCallback(async () => {
    try {
      ...
      if (numericUserId) {
        // Fetch contact_type alongside contact_id
        favoritesRes = await supabase.from("favorite_chats").select("contact_id, contact_type").eq("user_id", numericUserId);
      }
      ...
      try {
        // Build a composite key for favorites: "type_id"
        const favoriteIds = (favoritesRes.data || []).map((row) => `${row.contact_type || 'alumni'}_${row.contact_id}`);
  ...
  const activeChats = useMemo(() => {
    ...
    if (selectedTab === "favorites") {
      const favoriteChats = contacts
        .filter((item) => favoriteContactIds.has(`${item?.user_type || 'alumni'}_${item?.connection_id}`)) 
    ...
    const mergedChats = [
      ...
      ...contacts.map((contact) => ({ ...contact, __chatType: "contact", is_favorite: favoriteContactIds.has(`${contact?.user_type || 'alumni'}_${contact?.connection_id}`) })),
    ];
  ...
  const renderContactItem = ({ item }) => {
    ...
    const isFavorited = favoriteContactIds.has(`${item?.user_type || 'alumni'}_${item?.connection_id}`); 
  ...
  // --- FIXED: Safe UPSERT handling to avoid DB constraint crashes ---
  const updateDMSettings = async (contactId, updates) => {
    const userId = currentUserProfile?.id;
    if (!userId || !contactId) return;
    
    const contactType = modalContact?.user_type || 'alumni';
    ...
    try {
      ...
      if (existingData && existingData.length > 0) {
        ...
        const { error: insertError } = await supabase
          .from("dm_settings")
          .insert({ user_id: userId, contact_id: contactId, contact_type: contactType, ...updates });
  ...
  // --- FIXED: toggling favorite securely ---
  const handleToggleFavorite = async (contact) => {
    if (!contact?.connection_id) return;
    const contactId = contact.connection_id;
    const contactType = contact.user_type || 'alumni';
    const userId = currentUserProfile?.id;
    if (!userId) return;

    const compositeKey = `${contactType}_${contactId}`;
    const isFavorited = favoriteContactIds.has(compositeKey);
    const nextState = !isFavorited;
    const prevSet = new Set(favoriteContactIds);

    setFavoriteContactIds((prev) => {
      const next = new Set(prev);
      if (nextState) next.add(compositeKey);
    ...
    try {
      if (nextState) {
        const { error } = await supabase.from("favorite_chats").insert({ user_id: userId, contact_id: contactId, contact_type: contactType });
        if (error) throw error;
      } else {
        const { error } = await supabase.from("favorite_chats").delete().eq("user_id", userId).eq("contact_id", contactId).eq("contact_type", contactType);
        if (error) throw error;
      }
    } catch (e) {
          ...
          <SafeAreaView style={styles.actionSheetSafeArea} edges={["bottom"]}>
            <View style={styles.actionSheet}>
              ...
              <Pressable style={styles.actionItem} onPress={() => handleToggleFavorite(modalContact)}>
                <Ionicons 
                  name={favoriteContactIds.has(`${modalContact?.user_type || 'alumni'}_${modalContact?.connection_id}`) ? "star" : "star-outline"} 
                  size={20} 
                  color="#F2C919" 
                  style={styles.actionIcon} 
                />
                <Text style={[styles.actionText, { color: "#F2C919" }]}>
                  {favoriteContactIds.has(`${modalContact?.user_type || 'alumni'}_${modalContact?.connection_id}`) ? "Unfavorite" : "Favorite"}
                </Text>
              </Pressable>
