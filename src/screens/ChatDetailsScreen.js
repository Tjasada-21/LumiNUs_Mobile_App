...
const ChatDetailsScreen = ({ route, navigation }) => {
  ...
  const [groupAvatarDraft, setGroupAvatarDraft] = useState(
    routeGroup?.avatar_url ?? routeGroup?.avatar ?? "",
  );
  ...
  const normalizedMembers = useMemo(() => {
    ...
    return rawMembers.map((member, index) => {
      ...
      const fallbackName = [firstName, lastName]
        ...
        .trim();
      const fullName =
        (profile?.name ?? member?.name ?? fallbackName) || "Member";
      const avatar =
        profile?.avatar ??
        profile?.photo ??
  ...
  const isCurrentUserAdmin = useMemo(() => {
    ...
    return normalizedMembers.some(
      (member) =>
        String(member.alumniId) === String(currentUserId) &&
  ...
  const handlePickGroupAvatar = useCallback(async () => {
    ...
    if (!permissionResult.granted) {
      ThemedAlert.alert(
        "Permission needed",
        "Allow photo access to change the group avatar.",
      );
      return;
    }
    ...
    try {
      ...
      ThemedAlert.alert(
        "Upload failed",
        "Unable to update the group avatar right now.",
      );
    } finally {
  ...
  const handleSaveGroupDetails = useCallback(async () => {
    ...
    try {
      ...
      ThemedAlert.alert(
        "Group Updated",
        "The group name and avatar were saved.",
      );
    } catch (error) {
      console.error("[ChatDetails] Failed to update group details:", error);
      ThemedAlert.alert(
        "Update failed",
        "Unable to update the group right now.",
      );
    } finally {
      setIsSavingGroupDetails(false);
    }
          ...
          { text: "Cancel", style: "cancel" },
          {
            ...
            onPress: async () => {
              try {
                ...
                const fetchedGroup = await getGroupChat(routeGroupId).catch(
                  () => null,
                );
                if (fetchedGroup) {
                  applyFetchedGroup(fetchedGroup);
                  ...
                  setResolvedGroup((previous) => ({
                    ...(previous ?? {}),
                    members: (Array.isArray(previous?.members)
                      ? previous.members
                      : []
                    ).filter(
                      (item) =>
                        String(
                          item?.alumni_id ?? item?.alumni?.id ?? item?.id,
                        ) !== String(memberId),
                    ),
                  }));
                }
                ThemedAlert.alert(
                  "Member removed",
                  `${member?.name ?? "The member"} was removed from the group.`,
                );
              } catch (error) {
  ...
  const groupAvatarUri = getAvatarUri(
    groupData?.name,
  ...
  const handleAddMember = async (member) => {
    ...
    try {
      ...
      setCandidateMembers((previous) =>
        previous.filter((item) => String(item?.id) !== String(member.id)),
      );
      ThemedAlert.alert(
        "Member Added",
        `${member.name} was added to the group chat.`,
      );
    } catch (error) {
      ...
      if (message.includes("duplicate") || message.includes("already")) {
        ThemedAlert.alert(
          "Already a Member",
          "This alumni is already in the group chat.",
        );
      } else {
        ThemedAlert.alert(
          "Add Failed",
          "Unable to add member right now. Please try again.",
        );
      }
    } finally {
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              ThemedAlert.alert(
                "Error",
                "Could not leave the group at this time.",
              );
            }
          },
        },
  ...
  const handleAudioCall = () => {
    ThemedAlert.alert(
      "Coming Soon",
      "Audio calling will be available in a future update.",
    );
  };
  ...
  const submitReport = async (reason) => {
    try {
      ...
      ThemedAlert.alert(
        "Report Submitted",
        "Thank you. We have received your report and will review this account.",
      );
    }
  };
  ...
  const handleMute = () => {
    ThemedAlert.alert(
      "Mute messages",
      "Are you sure you want to mute notifications from this user?",
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Mute",
          ...
          >
            <View style={styles.dmCenterHeader}>
              <Image
                source={{
                  uri: getAvatarUri(
                    routeContact?.name ?? routeContact?.first_name,
                    routeContact?.avatar ?? routeContact?.alumni_photo,
                  ),
                }}
              ...
              <Text style={styles.dmName}>
                {routeContact?.name ??
                  (`${routeContact?.first_name ?? ""} ${routeContact?.last_name ?? ""}`.trim() ||
                    "Alumni")}
              </Text>
              {routeContact?.username ? (
                <Text style={styles.dmUsername}>@{routeContact.username}</Text>
              ) : null}
            ...
            <View style={styles.dmOptionsList}>
              ...
              <TouchableOpacity style={styles.dmOptionRow} onPress={handleMute}>
                <Ionicons
              ...
              >
                <Ionicons
                  ...
                  color="#DC2626"
                />
                <Text style={[styles.dmOptionText, { color: "#DC2626" }]}>
                  Block
                </Text>
              </TouchableOpacity>
      ...
      <View style={styles.container}>
          ...
          <View style={styles.groupActionsRow}>
            <TouchableOpacity
              style={[
                ...
                !canManageGroup ? styles.groupActionButtonMuted : null,
              ]}
            ...
            >
              <Ionicons
                name="create-outline"
                size={16}
                color={canManageGroup ? "#31429B" : "#9CA3AF"}
              />
              <Text
                style={[
                  styles.groupActionButtonText,
                  !canManageGroup ? styles.groupActionButtonTextMuted : null,
                ]}
              >
                Edit Group
              </Text>
            ...
            >
              ...
              <Text style={styles.groupActionButtonSecondaryText}>
                View Members
              </Text>
            </TouchableOpacity>
          </View>
        ...
        {isLoadingMembers ? (
          ...
          <Text style={styles.emptyText}>
            No members found for this group chat.
          </Text>
        )}
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalCard}>
              ...
              <View style={styles.editAvatarWrap}>
                <Image
                  source={{ uri: groupAvatarDraft || groupAvatarUri }}
                  style={styles.editAvatar}
                />
                <TouchableOpacity
                  style={[
                    styles.avatarEditButton,
                    !canManageGroup || isSavingGroupDetails
                      ? styles.groupActionButtonMuted
                      : null,
                  ]}
                ...
                >
                  ...
                  <Text
                    style={[
                      styles.avatarEditButtonText,
                      !canManageGroup
                        ? styles.groupActionButtonTextMuted
                        : null,
                    ]}
                  >
                    Change Avatar
              ...
              <TouchableOpacity
                style={[
                  ...
                  isSavingGroupDetails ? styles.saveGroupButtonDisabled : null,
                ]}
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalCard}>
              ...
              <Text style={styles.modalSubTitle}>
                {normalizedMembers.length} people in this group
              </Text>
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <View style={styles.memberManageRow}>
                    ...
                    {canManageGroup &&
                    String(item.alumniId) !== String(currentUserId) ? (
                      <TouchableOpacity
                        style={[
                          styles.kickButton,
                ...
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <View style={styles.candidateRow}>
                      ...
                      <TouchableOpacity
                        style={[
                          styles.candidateAddButton,
                          addingMemberId === item.id
                            ? styles.candidateAddButtonDisabled
                            : null,
                        ]}
...
const styles = StyleSheet.create({
  ...
  container: {
    flex: 1,
    backgroundColor: "#fff",
    padding: Math.max(14, Math.min(20, SCREEN_WIDTH * 0.04)),
  },
  header: {
    alignItems: "center",
    marginBottom: Math.max(14, Math.min(22, SCREEN_HEIGHT * 0.024)),
  },
  backButton: { position: "absolute", left: 0, top: 0, zIndex: 2, padding: 4 },
  avatar: {
    width: Math.max(64, Math.min(84, SCREEN_WIDTH * 0.18)),
    height: Math.max(64, Math.min(84, SCREEN_WIDTH * 0.18)),
    borderRadius: Math.max(32, Math.min(42, SCREEN_WIDTH * 0.09)),
    marginBottom: 8,
  },
  name: {
    fontWeight: "bold",
    fontSize: Math.max(18, Math.min(22, SCREEN_WIDTH * 0.05)),
    color: "#222",
  },
  adminBadge: {
    marginTop: 6,
  ...
  groupActionButtonSecondaryText: {
    color: "#31429B",
    fontSize: 12,
    fontWeight: "700",
  },
  ...
  memberAvatar: {
    width: Math.max(36, Math.min(44, SCREEN_WIDTH * 0.1)),
    height: Math.max(36, Math.min(44, SCREEN_WIDTH * 0.1)),
    borderRadius: Math.max(18, Math.min(22, SCREEN_WIDTH * 0.05)),
  },
  memberName: {
    fontSize: Math.max(12, Math.min(14, SCREEN_WIDTH * 0.034)),
    color: "#222",
    marginLeft: 10,
    flexShrink: 1,
  },
  ...
  mediaImage: {
    width: Math.max(56, Math.min(80, SCREEN_WIDTH * 0.18)),
    height: Math.max(56, Math.min(80, SCREEN_WIDTH * 0.18)),
    borderRadius: 8,
    marginRight: 8,
  },
  leaveBtn: {
  ...
  leaveBtnText: {
    ...
    fontWeight: "bold",
    marginLeft: 6,
    fontSize: Math.max(14, Math.min(16, SCREEN_WIDTH * 0.036)),
  },
  ...
  bigAvatar: {
    width: Math.max(120, Math.min(160, SCREEN_WIDTH * 0.36)),
    height: Math.max(120, Math.min(160, SCREEN_WIDTH * 0.36)),
    borderRadius: Math.max(60, Math.min(80, SCREEN_WIDTH * 0.18)),
    marginBottom: 12,
  },
  dmName: {
    fontWeight: "800",
    fontSize: Math.max(18, Math.min(22, SCREEN_WIDTH * 0.06)),
    color: "#1F2937",
  },
