...
const CreatePostScreen = () => {
  ...
  const mentionContext = useMemo(() => {
    ...
    const cursor =
      selection && typeof selection.start === "number"
        ? selection.start
        : text.length;
  ...
  const handlePickVideo = () =>
    handlePickMedia(
      ...
      setIsPickingVideo,
      setSelectedVideoUris,
      "Permission to access photos is required to choose a video.",
    );
  ...
  const handleRemovePhoto = (uriToRemove) => {
    ...
    setSelectedPhotoFiles((currentFiles) =>
      currentFiles.filter((file) => file.uri !== uriToRemove),
    );
  };

  const handleRemoveExistingPhoto = (imageId) => {
    setRemovedExistingPhotoIds((currentIds) =>
      currentIds.includes(imageId) ? currentIds : [...currentIds, imageId],
    );
  };

  const handleRemoveVideo = (uriToRemove) => {
    setSelectedVideoUris((currentUris) =>
      currentUris.filter((uri) => uri !== uriToRemove),
    );
  };
  ...
  const handleSubmitPost = async (isDraft = false) => {
    ...
    try {
      ...
      if (!supaUser) {
        ThemedAlert.alert(
          "Sign in required",
          "Please sign in again before creating a post.",
          [{ text: "OK" }],
          { variant: "error" },
        );
        return;
      ...
      const mentionMap = new Map(
        (connections || []).map((c) => [
          toMentionHandle(c?.first_name ?? "", c?.last_name ?? ""),
          c?.id,
        ]),
      );
      const mentionedIds = Array.from(foundHandles)
        .map((h) => mentionMap.get(h))
        .filter(Boolean);
      ...
      ThemedAlert.alert(
        "Upload failed",
        isEditMode
          ? `Unable to update your post: ${errorMsg}`
          : `Unable to create your post: ${errorMsg}`,
        [{ text: "OK" }],
        { variant: "error" },
      );
    } finally {
            ...
            <View style={styles.card}>
              ...
              {isEditMode ? (
                <Text style={styles.editNotice}>
                  Editing this post. Keep the current images or add new ones
                  below.
                </Text>
              ) : null}
                  ...
                  {audienceOptions.map((option) => {
                    ...
                    return (
                      <Pressable
                        ...
                        style={[
                          styles.audiencePill,
                          isSelected && styles.audiencePillSelected,
                        ]}
                      ...
                      >
                        ...
                        {isSelected ? (
                          <Ionicons
                            ...
                            size={14}
                            color="#31429B"
                            style={styles.audienceCheckIcon}
                          />
                        ) : null}
                      </Pressable>
                    );
                ...
                <View style={styles.previewImageWrap}>
                  {selectedLocation ? (
                    <View style={styles.locationPreviewRow}>
                      ...
                      />
                      <Text
                        style={styles.locationPreviewText}
                        numberOfLines={1}
                      >
                  ...
                  {previewPhotoItems.length > 0 ? (
                    ...
                    <View style={styles.previewMediaItem}>
                      ...
                      <Pressable
                        style={styles.previewRemoveButton}
                        onPress={() => handleRemoveVideo(selectedVideoUris[0])}
                      >
                        <Ionicons name="close" size={14} color="#FFFFFF" />
                      </Pressable>
                    </View>
              ...
              <View style={styles.footerRow}>
                ...
                <Pressable
                  style={[
                    styles.postButton,
                    isPublishDisabled && styles.postButtonDisabled,
                  ]}
