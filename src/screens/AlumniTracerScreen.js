...
const getStatus = (i) => {
  ...
  return s === undefined
    ? "Inactive"
    : String(s) === "1" || String(s).toLowerCase() === "active"
      ? "Active"
      : "Inactive";
};
const fmtKey = (k) =>
    ...
    >
      ...
      <View style={styles.cardInfo}>
        ...
        <View
          style={[
            styles.statusBadge,
            completed ? styles.statusBadgeActive : styles.statusBadgeInactive,
          ]}
        >
          <Text
            style={[
...
const AlumniTracerScreen = () => {
  ...
  const fetchTracers = useCallback(async () => {
    try {
      ...
      const alumni = supaUser?.email
        ? await getAlumniByEmail(supaUser.email).catch(() => null)
        : null;
      const responses = alumni?.id
        ? await getUserTracerResponses(alumni.id).catch(() => [])
        : [];
  ...
  const checkUserResponse = useCallback(async (formId) => {
    ...
    try {
      ...
      const answered = responses.some(
        (response) =>
          String(response?.form?.id || response?.form_id) === String(formId),
      );
      const draft = answered
        ? null
        : await getDraftResponse(alumni.id, formId).catch(() => null);
      if (isMounted.current) {
  ...
  const renderItem = useCallback(
    ({ item }) => (
      <TracerItem
        item={item}
        onPress={openDetails}
        completed={answeredFormIds.has(getFormId(item))}
      />
    ),
  ...
  const renderSectionHeader = useCallback(
    ({ section: { title, data } }) => (
      <View style={styles.sectionHeaderWrap}>
        {title === "Completed" ? (
          <Text style={styles.sectionHeaderLabel}>Tracers</Text>
        ) : null}
          ...
          <View style={styles.container}>
            ...
            <Pressable
              style={[
                ...
                hasAnswered && styles.modalActionButtonDisabled,
                {
                  flexDirection: "row",
                  alignItems: "center",
            ...
            >
              ...
              <Text
                style={[
                  ...
                  hasAnswered && styles.modalActionTextDisabled,
                ]}
              >
                {checkingResponse
                  ? "Checking..."
